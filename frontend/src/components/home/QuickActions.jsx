...
import { Calendar, FileText, Stethoscope, Phone, Ambulance, MessageCircle } from "lucide-react";
const actions = [
    {
        icon: Calendar,
    ...
    },
    {
        ...
        description: "Your AI skincare companion",
...
export const QuickActions = () => {
    ...
    return (<section className="py-20 lg:py-32 bg-background" ref={ref}>
       <div className="container mx-auto px-4 lg:px-8">
         <motion.div initial={{ opacity: 0, x: -30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.6 }} className="text-center mb-16">
         ...
         <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
           {actions.map((action, index) => (<motion.div key={action.title} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: index * 0.1 }}>
               <Link to={action.link} className="group block p-6 lg:p-8 rounded-2xl bg-card border border-border hover:border-accent/50 hover:shadow-xl hover:-translate-y-2 transition-all duration-300">
