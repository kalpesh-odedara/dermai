   ...
   return (
     <section className="relative h-[90vh] lg:h-screen overflow-hidden">
       ...
       <div className="absolute bottom-8 right-8 flex items-center gap-4 z-20">
         <button
           onClick={prev}
           className="w-12 h-12 rounded-full bg-card/20 backdrop-blur-sm border border-primary-foreground/20 flex items-center justify-center text-primary-foreground hover:bg-card/40 transition-all"
         >
         ...
         {slides.map((_, index) => (
           <button
             ...
             className={`h-2 rounded-full transition-all duration-300 ${
               index === current ? "w-8 bg-accent" : "w-2 bg-primary-foreground/40"
             }`}
           />
         ))}
       </div>
       ...
       <motion.div
         ...
         className="absolute bottom-0 left-0 right-0 bg-card/90 backdrop-blur-xl border-t border-border hidden lg:block"
       >
         <div className="container mx-auto px-8">
           <div className="grid grid-cols-3 divide-x divide-border">
             {[
               ...
               <div key={index} className="flex items-center gap-4 py-6 px-8">
